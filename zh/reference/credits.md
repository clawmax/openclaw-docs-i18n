

  项目

  
# 致谢

## 名称由来

OpenClaw = CLAW + TARDIS，因为每只太空龙虾都需要一台时间和空间机器。

## 致谢名单

-   **Peter Steinberger** ([@steipete](https://x.com/steipete)) - 创建者，龙虾耳语者
-   **Mario Zechner** ([@badlogicc](https://x.com/badlogicgames)) - Pi 创建者，安全渗透测试员
-   **Clawd** - 那只要求取个更好名字的太空龙虾

## 核心贡献者

-   **Maxim Vovshin** (@Hyaxia, [36747317+Hyaxia@users.noreply.github.com](mailto:36747317+Hyaxia@users.noreply.github.com)) - Blogwatcher 技能
-   **Nacho Iacovino** (@nachoiacovino, [nacho.iacovino@gmail.com](mailto:nacho.iacovino@gmail.com)) - 位置解析 (Telegram 和 WhatsApp)
-   **Vincent Koc** ([@vincentkoc](https://github.com/vincentkoc), [@vincent\_koc](https://x.com/vincent_koc)) - 智能体、遥测、钩子、安全

## 许可证

MIT - 自由如海洋中的龙虾。

> “我们都只是在玩自己的提示词。”（某个AI，可能正沉迷于令牌）

[时区](../concepts/timezone.md)[发布清单](./RELEASING.md)

---