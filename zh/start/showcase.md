

  概述

  
# 展示

来自社区的真实项目。看看人们正在用 OpenClaw 构建什么。

> **ℹ️** **想要被展示吗？** 在 Discord 的 [#showcase 频道](https://discord.gg/clawd) 分享您的项目，或在 X 上 [@openclaw](https://x.com/openclaw)。

## 🎥 OpenClaw 实战

VelvetShark 的完整设置演练 (28分钟)。

[在 YouTube 上观看](https://www.youtube.com/watch?v=SaWSPZoPX34)

[在 YouTube 上观看](https://www.youtube.com/watch?v=mMSKQvlmFuQ)

[在 YouTube 上观看](https://www.youtube.com/watch?v=5kkIJNUGFho)

## 🆕 Discord 新鲜事

## 🤖 自动化与工作流

## 🧠 知识与记忆

## 🎙️ 语音与电话

## 🏗️ 基础设施与部署

## 🏠 家庭与硬件

## 🌟 社区项目

* * *

## 提交您的项目

有东西要分享吗？我们很乐意展示它！

### 步骤 1：分享它

发布到 Discord 的 [#showcase 频道](https://discord.gg/clawd) 或在 X 上 [@openclaw](https://x.com/openclaw)

### 步骤 2：包含详细信息

告诉我们它有什么功能，链接到仓库/演示，如果有的话分享一张截图

### 步骤 3：获得展示

我们会将出色的项目添加到此页面

[OpenClaw](../home.md)[功能](../concepts/features.md)