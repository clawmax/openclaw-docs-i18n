...
最后更新：2026-01-10 TypeBox 是一个 TypeScript 优先的模式库。我们用它来定义**网关 WebSocket 协议**（握手、请求/响应、服务器事件）。这些模式驱动着**运行时验证**、**JSON Schema 导出**以及 macOS 应用的 **Swift 代码生成**。单一事实来源；其他一切都是生成的。如果你想了解更高层次的协议背景，请从[网关架构](./architecture.md)开始。
...
第一帧**必须**是 `connect` 请求。之后，客户端可以调用方法（例如 `health`、`send`、`chat.send`）并订阅事件（例如 `presence`、`tick`、`agent`）。连接流程（最小化）：
...
| 消息传递 | `send`, `poll`, `agent`, `agent.wait` | 有副作用的操作需要 `idempotencyKey` |
...
-   **客户端**：JS 客户端在使用事件和响应帧之前会验证它们。
-   **方法接口**：网关在 `hello-ok` 中通告支持的 `methods` 和 `events`。
...
在 `src/gateway/server-methods.ts` 中注册它（已经合并了 `systemHandlers`），然后在 `src/gateway/server.ts` 中将 `"system.echo"` 添加到 `METHODS`。
...
-   带有 `req`、`res`、`event` 和 `unknown` 枚举项的 `GatewayFrame` 枚举
-   强类型的负载结构体/枚举
-   `ErrorCode` 值和 `GATEWAY_PROTOCOL_VERSION`

未知的帧类型将作为原始负载保留，以实现向前兼容。
...
-   大多数对象使用 `additionalProperties: false` 以实现严格的负载。
-   `NonEmptyString` 是 ID 和方法/事件名称的默认类型。
-   顶层的 `GatewayFrame` 在 `type` 上使用**鉴别器**。
-   具有副作用的方法通常需要在参数中包含 `idempotencyKey`（例如：`send`、`poll`、`agent`、`chat.send`）。
-   `agent` 接受可选的 `internalEvents`，用于运行时生成的编排上下文（例如子代理/定时任务完成交接）；将此视为内部 API 接口。
...
[日期和时间](../date-time.md)[Markdown 格式化](./markdown-formatting.md)