...
使用 Docker 在 Hetzner VPS 上运行一个持久化的 OpenClaw Gateway，具备持久状态、内置二进制文件和安全的重启行为。如果你想要“24/7 运行的 OpenClaw，约 5 美元”，这是最简单可靠的设置。Hetzner 价格会变动；选择最小的 Debian/Ubuntu VPS，如果遇到内存不足（OOM）再升级。安全模型提醒：

-   当所有人都在同一个信任边界内且运行时环境仅用于业务时，公司共享的代理是可以的。
...
如果你使用其他 Linux VPS，请相应映射软件包。关于通用的 Docker 流程，请参阅[Docker](./docker.md)。
...
services:
  openclaw-gateway:
    ...
    ports:
      # 推荐：在 VPS 上保持 Gateway 仅限本地回环访问；通过 SSH 隧道访问。
