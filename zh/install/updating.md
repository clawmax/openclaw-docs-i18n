...
我们不建议将 Bun 用于网关运行时（WhatsApp/Telegram 存在 bug）。要切换更新通道（git + npm 安装）：
...
使用 `--tag <dist-tag|version>` 进行一次性安装指定标签/版本。有关通道语义和发布说明，请参阅[开发通道](./development-channels.md)。注意：在 npm 安装中，网关启动时会记录更新提示（检查当前通道标签）。可通过 `update.checkOnStart: false` 禁用。
