...
-   `resetArchiveRetention`: retention for `*.reset.` transcript archives (default: same as `pruneAfter`; `false` disables cleanup)
...
The canonical rules are documented at [/concepts/session](../concepts/session.md).
...
For more, see [/token-use](./token-use.md).
...
Compaction is **persistent** (unlike session pruning). See [/concepts/session-pruning](../concepts/session-pruning.md).
...
-   See [Memory](../concepts/memory.md) for the workspace file layout and write patterns.
...
-   Session key wrong? Start with [/concepts/session](../concepts/session.md) and confirm the `sessionKey` in `/status`.
...
[Node.js](../install/node.md)[Setup](../start/setup.md)
