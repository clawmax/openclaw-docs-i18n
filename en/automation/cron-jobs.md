...
> **Cron vs Heartbeat?** See [Cron vs Heartbeat](./cron-vs-heartbeat.md) for guidance on when to use each.

Cron is the Gateway’s built-in scheduler. It persists jobs, wakes the agent at the right time, and can optionally deliver output back to a chat. If you want *“run this every morning”* or *“poke the agent in 20 minutes”*, cron is the mechanism. Troubleshooting: [/automation/troubleshooting](./troubleshooting.md)
...
For the canonical JSON shapes and examples, see [JSON schema for tool calls](./cron-jobs.md#json-schema-for-tool-calls).
...
This is the best fit when you want the normal heartbeat prompt + main-session context. See [Heartbeat](../gateway/heartbeat.md).

#### Isolated jobs (dedicated cron sessions)

Isolated jobs run a dedicated agent turn in session `cron:`. Key behaviors:
...
Configure `cron.retry` to override these defaults (see [Configuration](./cron-jobs.md#configuration)).
...
-   `cron.run` (force or due), `cron.runs` For immediate system events without a job, use [`openclaw system event`](../cli/system.md).
...
-   One-shot (`at`) jobs retry transient errors (rate limit, overloaded, network, server\_error) up to 3 times with backoff; permanent errors disable immediately. See [Retry policy](./cron-jobs.md#retry-policy).
