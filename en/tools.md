...
Plugins can register **additional tools** (and CLI commands) beyond the core set. See [Plugins](./tools/plugin.md) for install + config, and [Skills](./tools/skills.md) for how tool usage guidance is injected into prompts. Some plugins ship their own skills alongside tools (for example, the voice-call plugin). Optional plugin tools:

-   [Lobster](./tools/lobster.md): typed workflow runtime with resumable approvals (requires the Lobster CLI on the gateway host).
-   [LLM Task](./tools/llm-task.md): JSON-only LLM step for structured workflow output (optional schema validation).
-   [Diffs](./tools/diffs.md): read-only diff viewer and PNG or PDF file renderer for before/after text or unified patches.
...
-   gateway/node approvals and allowlists: [Exec approvals](./tools/exec-approvals.md).
...
-   See [Web tools](./tools/web.md) for setup.
...
-   See [Web tools](./tools/web.md) for setup.
-   See [Firecrawl](./tools/firecrawl.md) for the optional anti-bot fallback.
...
Analyze one or more PDF documents. For full behavior, limits, config, and examples, see [PDF tool](./tools/pdf.md).
...
-   `sessions_spawn` supports `runtime: "subagent" | "acp"` (`subagent` default). For ACP runtime behavior, see [ACP Agents](./tools/acp-agents.md).
