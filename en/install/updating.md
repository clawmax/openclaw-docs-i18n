...
Use `--tag <dist-tag|version>` for a one-off install tag/version. See [Development channels](./development-channels.md) for channel semantics and release notes. Note: on npm installs, the gateway logs an update hint on startup (checks the current channel tag). Disable via `update.checkOnStart: false`.
...
Details: [Doctor](../gateway/doctor.md)
...
-   macOS launchd (app-bundled LaunchAgent): `launchctl kickstart -k gui/$UID/ai.openclaw.gateway` (use `ai.openclaw.`; legacy `com.openclaw.*` still works)
...
Runbook + exact service labels: [Gateway runbook](../gateway.md)
...
-   Check: [Troubleshooting](../gateway/troubleshooting.md)
-   Ask in Discord: [https://discord.gg/clawd](https://discord.gg/clawd)

[Bun (Experimental)](./bun.md)[Migration Guide](./migrating.md)
