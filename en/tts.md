...
TTS config lives under `messages.tts` in `openclaw.json`. Full schema is in [Gateway configuration](./gateway/configuration.md).
...
There is a single command: `/tts`. See [Slash commands](./tools/slash-commands.md) for enablement details. Discord note: `/tts` is a built-in Discord command, so OpenClaw registers `/voice` as the native command there. Text `/tts ...` still works.
