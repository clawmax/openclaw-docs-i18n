...
-   You can override per-message (`/think:`) or in model params: `agents.defaults.models["anthropic/"].params.thinking`.
...
We recommend migrating to the new `cacheRetention` parameter. OpenClaw includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API requests; keep it if you override provider headers (see [/gateway/configuration](../gateway/configuration.md)).
...
-   If you see “OAuth token refresh failed …” on a Claude subscription, re-auth with a setup-token. See [/gateway/troubleshooting#oauth-token-refresh-failed-anthropic-claude-subscription](../gateway/troubleshooting.md#oauth-token-refresh-failed-anthropic-claude-subscription).
-   Auth details + reuse rules are in [/concepts/oauth](../concepts/oauth.md).
...
More: [/gateway/troubleshooting](../gateway/troubleshooting.md) and [/help/faq](../help/faq.md).

[Model Failover](../concepts/model-failover.md)[Amazon Bedrock](./bedrock.md)
