...
OpenAI’s API exposes priority processing via `service_tier=priority`. In OpenClaw, set `agents.defaults.models["openai/"].params.serviceTier` to pass that field through on direct `openai/*` Responses requests.
...
-   Model refs always use `provider/model` (see [/concepts/models](../concepts/models.md)).
