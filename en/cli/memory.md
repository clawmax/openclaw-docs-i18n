...
-   Memory concept: [Memory](../concepts/memory.md)
...
-   `--agent `: scope to a single agent. Without it, these commands run for each configured agent; if no agent list is configured, they fall back to the default agent.
...
-   Query input: pass either positional `[query]` or `--query `.
-   If both are provided, `--query` wins.
-   If neither is provided, the command exits with an error.
-   `--agent `: scope to a single agent (default: the default agent).
-   `--max-results `: limit the number of results returned.
-   `--min-score `: filter out low-score matches.
