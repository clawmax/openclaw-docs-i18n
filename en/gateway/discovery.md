...
-   **Legacy TCP bridge (deprecated/removed)**: older node transport (see [Bridge protocol](./bridge-protocol.md)); no longer advertised for discovery.

Protocol details:

-   [Gateway protocol](./protocol.md)
-   [Bridge protocol (legacy)](./bridge-protocol.md)
...
Troubleshooting and beacon details: [Bonjour](./bonjour.md).
...
When there is no direct route (or direct is disabled), clients can always connect via SSH by forwarding the loopback gateway port. See [Remote access](./remote.md).
...
-   Pairing requests are created/approved/rejected in the gateway (see [Gateway pairing](./pairing.md)).
...
[Gateway-Owned Pairing](./pairing.md)[Bonjour Discovery](./bonjour.md)
