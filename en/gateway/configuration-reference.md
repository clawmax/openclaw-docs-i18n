...
Every field available in `~/.openclaw/openclaw.json`. For a task-oriented overview, see [Configuration](./configuration.md). Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenClaw uses safe defaults when omitted.
...
-   Top-level `bindings[]` entries with `type: "acp"` configure persistent ACP bindings for forum topics (use canonical `chatId:topic:topicId` in `match.peer.id`). Field semantics are shared in [ACP Agents](../tools/acp-agents.md#channel-specific-settings).
-   Telegram stream previews use `sendMessage` + `editMessageText` (works in direct and group chats).
-   Retry policy: see [Retry policy](../concepts/retry.md).
...
-   Top-level `bindings[]` entries with `type: "acp"` configure persistent ACP bindings for channels and threads (use channel/thread id in `match.peer.id`). Field semantics are shared in [ACP Agents](../tools/acp-agents.md#channel-specific-settings).
...
-   Full BlueBubbles channel configuration is documented in [BlueBubbles](../channels/bluebubbles.md).
...
-   Full Teams config (credentials, webhook, DM/group policy, per-team/per-channel overrides) is documented in [Microsoft Teams](../channels/msteams.md).
...
-   Full IRC channel configuration (host/port/TLS/channels/allowlists/mention gating) is documented in [IRC](../channels/irc.md).
...
Many extension channels are configured as `channels.` and documented in their dedicated channel pages (for example Feishu, Matrix, LINE, Nostr, Zalo, Nextcloud Talk, Synology Chat, and Twitch). See the full channel index: [Channels](../channels.md).
...
-   `mode`: `default` or `safeguard` (chunked summarization for long histories). See [Compaction](../concepts/compaction.md).
...
-   If fewer than `keepLastAssistants` assistant messages exist, pruning is skipped.
 ...
 See [Session Pruning](../concepts/session-pruning.md) for behavior details.
...
See [Streaming](../concepts/streaming.md) for behavior + chunking details.
...
See [Typing Indicators](../concepts/typing-indicators.md).

### agents.defaults.sandbox

Optional **Docker sandboxing** for the embedded agent. See [Sandboxing](./sandboxing.md) for the full guide.
...
-   Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
    ...
    -   `--renderer-process-limit=2` can be changed with `OPENCLAW_BROWSER_RENDERER_PROCESS_LIMIT=`; set `0` to use Chromium’s default process limit.
...
Run multiple isolated agents inside one Gateway. See [Multi-Agent](../concepts/multi-agent.md).
...
```
 ...
 See [Multi-Agent Sandbox & Tools](../tools/multi-agent-sandbox-tools.md) for precedence details.
...
-   **`maintenance`**: session-store cleanup + retention controls.
    ...
    -   `resetArchiveRetention`: retention for `*.reset.` transcript archives. Defaults to `pruneAfter`; set `false` to disable.
...
Per-channel/account overrides: `channels..responsePrefix`, `channels..accounts..responsePrefix`. Resolution (most specific wins): account → channel → global. `""` disables and stops cascade. `"auto"` derives `[{identity.name}]`. **Template variables:**
...
See [Local Models](./local-models.md). TL;DR: run MiniMax M2.5 via LM Studio Responses API on serious hardware; keep hosted models merged for fallback.
...
-   `gateway.auth.mode: "trusted-proxy"`: delegate auth to an identity-aware reverse proxy and trust identity headers from `gateway.trustedProxies` (see [Trusted Proxy Auth](./trusted-proxy-auth.md)).
...
-   Optional response hardening header:
    -   `gateway.http.securityHeaders.strictTransportSecurity` (set only for HTTPS origins you control; see [Trusted Proxy Auth](./trusted-proxy-auth.md#tls-termination-and-hsts))
...
Convenience flags: `--dev` (uses `~/.openclaw-dev` + port `19001`), `--profile ` (uses `~/.openclaw-`). See [Multiple Gateways](./multiple-gateways.md).
...
Auth: `Authorization: Bearer ` or `x-openclaw-token: `. **Endpoints:**
...
-   See [Environment](../help/environment.md) for full precedence.
...
-   Canonical matrix: [SecretRef Credential Surface](../reference/secretref-credential-surface.md)
...
-   See [OAuth](../concepts/oauth.md).
-   Secrets runtime behavior and `audit/configure/apply` tooling: [Secrets Management](./secrets.md).
...
See [Cron Jobs](../automation/cron-jobs.md).
...
*Related: [Configuration](./configuration.md) · [Configuration Examples](./configuration-examples.md) · [Doctor](./doctor.md)*

[Configuration](./configuration.md)[Configuration Examples](./configuration-examples.md)
