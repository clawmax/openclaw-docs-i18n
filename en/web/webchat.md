...
-   The Control UI `/agents` Tools panel fetches a runtime catalog via `tools.catalog` and labels each tool as `core` or `plugin:` (plus `optional` for optional plugin tools).
...
Full configuration: [Configuration](../gateway/configuration.md) Channel options:
...
-   `gateway.auth.mode: "trusted-proxy"`: reverse-proxy auth for browser clients (see [Trusted Proxy Auth](../gateway/trusted-proxy-auth.md)).
