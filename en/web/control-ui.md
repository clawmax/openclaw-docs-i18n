...
Once approved, the device is remembered and won’t require re-approval unless you revoke it with `openclaw devices revoke --device  --role `. See [Devices CLI](../cli/devices.md) for token rotation and revocation. **Notes:**
...
`dangerouslyDisableDeviceAuth` disables Control UI device identity checks and is a severe security downgrade. Revert quickly after emergency use. See [Tailscale](../gateway/tailscale.md) for HTTPS setup guidance.
...
Remote access setup details: [Remote access](../gateway/remote.md).

[Web](../web.md)[Dashboard](./dashboard.md)
