...
If the node is showing the Canvas (WebView), `canvas.snapshot` returns `{ format, base64 }`. CLI helper (writes to a temp file and prints `MEDIA:`):
...
-   Exec approvals are enforced locally via `~/.openclaw/exec-approvals.json` (see [Exec approvals](./tools/exec-approvals.md)).
...
[Auth Monitoring](./automation/auth-monitoring.md)[Node Troubleshooting](./nodes/troubleshooting.md)
