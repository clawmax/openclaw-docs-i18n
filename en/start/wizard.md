...
> **ℹ️** Fastest first chat: open the Control UI (no channel setup needed). Run `openclaw dashboard` and chat in the browser. Docs: [Dashboard](../web/dashboard.md).
...
> **ℹ️** `--json` does not imply non-interactive mode. For scripts, use `--non-interactive`.

 

> **💡** The onboarding wizard includes a web search step where you can pick a provider (Perplexity, Brave, Gemini, Grok, or Kimi) and paste your API key so the agent can use `web_search`. You can also configure this later with `openclaw configure --section web`. Docs: [Web tools](../tools/web.md).
...
-   DM isolation default: local onboarding writes `session.dmScope: "per-channel-peer"` when unset. Details: [CLI Onboarding Reference](./wizard-cli-reference.md#outputs-and-internals)
...
> **ℹ️** Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset** (or pass `--reset`). CLI `--reset` defaults to config, credentials, and sessions; use `--reset-scope full` to include workspace. If the config is invalid or contains legacy keys, the wizard asks you to run `openclaw doctor` first.
...
Use `openclaw agents add ` to create a separate agent with its own workspace, sessions, and auth profiles. Running without `--workspace` launches the wizard. What it sets:
...
For detailed step-by-step breakdowns, non-interactive scripting, Signal setup, RPC API, and a full list of config fields the wizard writes, see the [Wizard Reference](../reference/wizard.md).

## Related docs

-   CLI command reference: [`openclaw onboard`](../cli/onboard.md)
-   Onboarding overview: [Onboarding Overview](./onboarding-overview.md)
-   macOS app onboarding: [Onboarding](./onboarding.md)
-   Agent first-run ritual: [Agent Bootstrapping](./bootstrapping.md)

[Onboarding Overview](./onboarding-overview.md)[Onboarding: macOS App](./onboarding.md)
