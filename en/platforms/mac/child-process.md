...
-   The app installs a per‑user LaunchAgent labeled `ai.openclaw.gateway` (or `ai.openclaw.` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.openclaw.*` is supported).
...
Replace the label with `ai.openclaw.` when running a named profile.
...
[Canvas](./canvas.md)[Health Checks](./health.md)
