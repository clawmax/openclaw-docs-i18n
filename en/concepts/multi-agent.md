...
See channel guides: [Discord](../channels/discord.md), [Telegram](../channels/telegram.md), [WhatsApp](../channels/whatsapp.md).
...
-   For shared groups, bind the group to one agent or use [Broadcast groups](../channels/broadcast-groups.md).
...
Channels that support **multiple accounts** (e.g. WhatsApp) use `accountId` to identify each login. Each `accountId` can be routed to a different agent, so one server can host multiple phone numbers without mixing sessions. If you want a channel-wide default account when `accountId` is omitted, set `channels..defaultAccount` (optional). When unset, OpenClaw falls back to `default` if present, otherwise the first configured account id (sorted). Common channels supporting this pattern include:
