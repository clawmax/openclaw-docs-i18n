...
OpenClaw treats **one direct-chat session per agent** as primary. Direct chats collapse to `agent::` (default `main`), while group/channel chats get their own keys. `session.mainKey` is honored. Use `session.dmScope` to control how **direct messages** are grouped:
...
-   You can verify your DM settings with `openclaw security audit` (see [security](../cli/security.md)).
...
OpenClaw trims **old tool results** from the in-memory context right before LLM calls by default. This does **not** rewrite JSONL history. See [/concepts/session-pruning](./session-pruning.md).

## Pre-compaction memory flush

When a session nears auto-compaction, OpenClaw can run a **silent memory flush** turn that reminds the model to write durable notes to disk. This only runs when the workspace is writable. See [Memory](./memory.md) and [Compaction](./compaction.md).
...
-   Send `/compact` (optional instructions) as a standalone message to summarize older context and free up window space. See [/concepts/compaction](./compaction.md).
...
[Bootstrapping](../start/bootstrapping.md)[Session Pruning](./session-pruning.md)
