...
-   When verbose is on, agents that emit structured tool results (Pi, other JSON agents) send each tool call back as its own metadata-only message, prefixed with ` <tool-name>: ` when available (path/command). These tool summaries are sent as soon as each tool starts (separate bubbles), not as streaming deltas.
...
-   Elevated mode docs live in [Elevated mode](./elevated.md).
...
[Reactions](./reactions.md)[Web Tools](./web.md)
