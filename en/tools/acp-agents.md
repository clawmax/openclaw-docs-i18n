...
See also [Sub-agents](./subagents.md).
...
When OpenClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases. Direct acpx CLI usage can also target arbitrary adapters via `--agent `, but that raw escape hatch is an acpx CLI feature (not the normal OpenClaw `agentId` path).
...
See [Configuration Reference](../gateway/configuration-reference.md).
...
5.  If the plugin-local binary is missing or mismatched, it runs: `npm install --omit=dev --no-save acpx@` and re-verifies.
