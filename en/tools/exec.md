...
-   `tools.exec.safeBins`: stdin-only safe binaries that can run without explicit allowlist entries. For behavior details, see [Safe bins](./exec-approvals.md#safe-bins-stdin-only).
...
Do not treat `safeBins` as a generic allowlist, and do not add interpreter/runtime binaries (for example `python3`, `node`, `ruby`, `bash`). If you need those, use explicit allowlist entries and keep approval prompts enabled. `openclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `openclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries. For full policy details and examples, see [Exec approvals](./exec-approvals.md#safe-bins-stdin-only) and [Safe bins versus allowlist](./exec-approvals.md#safe-bins-versus-allowlist).
...
[Elevated Mode](./elevated.md)[Exec Approvals](./exec-approvals.md)
