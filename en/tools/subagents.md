...
-   For ACP harness sessions (Codex, Claude Code, Gemini CLI), use `sessions_spawn` with `runtime: "acp"` and see [ACP Agents](./acp-agents.md).
...
See [Configuration Reference](../gateway/configuration-reference.md) and [Slash commands](./slash-commands.md) for current adapter details. Allowlist:
