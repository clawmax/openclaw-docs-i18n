...
See [Voice Call](../plugins/voice-call.md) for a concrete example plugin. Looking for third-party listings? See [Community plugins](../plugins/community.md).
...
OpenClaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config validation does not execute plugin code**; it uses the plugin manifest and JSON Schema instead. See [Plugin manifest](../plugins/manifest.md). Plugins can register:
...
Plugins run **in‑process** with the Gateway, so treat them as trusted code. Tool authoring guide: [Plugin agent tools](../plugins/agent-tools.md).
...
Most bundled plugins must be enabled explicitly via `plugins.entries..enabled` or `openclaw plugins enable `. Default-on bundled plugin exceptions:
...
-   Merges optional plugin-provided config field hints under: `plugins.entries..config.`
...
Plugins can register **channel plugins** that behave like built‑in channels (WhatsApp, Telegram, etc.). Channel config lives under `channels.` and is validated by your channel plugin code.
...
Load the plugin (extensions dir or `plugins.load.paths`), restart the gateway, then configure `channels.` in your config.

### Agent tools

See the dedicated guide: [Plugin agent tools](../plugins/agent-tools.md).
...
Plugins can ship a skill in the repo (`skills//SKILL.md`). Enable it with `plugins.entries..enabled` (or other config gates) and ensure it’s present in your workspace/managed skills locations.
...
-   `openclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.openclaw/extensions//`, and enables it in config.
...
See [Voice Call](../plugins/voice-call.md) and `extensions/voice-call/README.md` for setup and usage.
...
[ClawHub](./clawhub.md)[Community plugins](../plugins/community.md)
